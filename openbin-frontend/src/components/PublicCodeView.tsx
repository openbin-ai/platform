...
// openapk-frontend with purple accents (duplicated components — keep in sync).
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded border border-zinc-800 bg-zinc-900/40 px-6 py-10 text-center">
        ...
        <button
          ...
          className="mt-1 rounded border border-amber-600 bg-amber-950/40 px-4 py-1.5 text-sm font-medium text-amber-200 hover:bg-amber-900/40"
        >
      ...
      <div className="flex min-h-0 flex-col border-r border-zinc-800">
        <div className="border-b border-zinc-800 p-2">
          <input
            ...
            className="w-full rounded border border-zinc-800 bg-zinc-900 px-2 py-1 text-xs text-zinc-100 placeholder:text-zinc-600 focus:border-amber-500 focus:outline-none"
          />
        </div>
                  ...
                  {(['pseudo', 'disasm'] as const).map((m) => (
                    <button
                      ...
                      className={`rounded px-2 py-0.5 ${
                        mode === m ? 'bg-amber-950/60 text-amber-200' : 'text-zinc-500 hover:text-zinc-300'
                      }`}
                    >
