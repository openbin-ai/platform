...
const POPULATABLE = new Set([
  // APK MAR + VRR populate targets — left in for projects switched between
  // kinds, or templates a user authored from an APK-side template.
  'static_findings', 'iocs',
  'vulnerabilities', 'attack_surface', 'network_security',
  // BIN MAR + VRR populate targets — see ProjectReportService.renderForSection.
  'bin_findings', 'bin_iocs',
  'bin_vulnerabilities', 'bin_attack_surface',
])
...
export function ReportEditor({
  ...
  async function download() {
    ...
    a.href = url
    a.download = `openbin-report-${projectId}.md`
    ...
    <div className={compact ? 'space-y-3' : 'space-y-6'}>
      <div>
        ...
        <div className={`mt-2 flex flex-wrap items-center gap-2 ${compact ? 'text-[11px]' : 'text-xs'} text-zinc-500`}>
          ...
          {/* Community share — only meaningful once locked, hides during draft. */}
          {locked && !inCommunity && (
            <button
              ...
              className={compact
                ? 'rounded border border-amber-600 bg-amber-900/40 px-2.5 py-1 text-xs font-medium text-amber-100 hover:bg-amber-900/60 disabled:opacity-50'
                : 'rounded border border-amber-600 bg-amber-900/40 px-4 py-1.5 text-sm font-medium text-amber-100 hover:bg-amber-900/60 disabled:opacity-50'}
            >
        ...
        {inCommunity && reportId && (
          <div className="mt-2 rounded border border-amber-900/60 bg-amber-950/30 px-3 py-2 text-xs text-amber-200">
...
// Modal for "Share with community". Mirrors the openapk-frontend
// implementation — author picks STIX malware-type + up to 8 tags before
// the report becomes anonymously readable. Backend snapshots these at
// publish-time; re-submitting overwrites.
  ...
  return (
    <ModalShell title="Share with community" onClose={onClose}>
      <p className="mb-4 text-xs text-zinc-500">
        ...
        Make sure you've reviewed our <Link to="/terms" target="_blank" className="text-amber-400 hover:underline">community terms</Link>.
      </p>
      ...
      <select
        ...
        className="mt-1.5 w-full rounded border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 focus:border-amber-500 focus:outline-none"
      >
      ...
      <p className="mt-0.5 text-xs text-zinc-500">
        Up to 8, 32 chars each. Help others find your report — <em>elf-stripped</em>, <em>linux-arm64</em>, <em>packed-upx</em>.
      </p>
      <div className="mt-1.5 flex items-center gap-2">
        <input
          ...
          maxLength={32}
          className="flex-1 rounded border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-amber-500 focus:outline-none"
        />
        <button
          type="button"
      ...
      <div className="mt-6 flex items-center justify-end gap-2 border-t border-zinc-800 pt-4">
        ...
        <button
          onClick={submit}
          disabled={busy}
          className="rounded border border-amber-600 bg-amber-900/40 px-4 py-1.5 text-sm text-amber-100 hover:bg-amber-800/60 disabled:opacity-50"
        >
