...
// Plain-English T&Cs for community-published reports. Identical to
// openapk-frontend/src/pages/Terms.tsx with OPENBIN branding.
    ...
    <div className="flex h-full flex-col bg-zinc-950 text-zinc-200">
      <header className="border-b border-zinc-800 bg-zinc-950">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          ...
          <Link to="/community" className="text-sm text-zinc-300 hover:text-zinc-100">
            Community
          </Link>
        </div>
      </header>
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
        ...
        <div className="prose prose-invert prose-sm max-w-none prose-headings:text-zinc-200 prose-a:text-amber-400 text-zinc-300">
          <p>
            When you publish a report to the OpenBin community, you make it visible to anyone
        ...
        <div className="mt-8 flex items-center gap-4 border-t border-zinc-900 pt-4 text-sm">
          <Link to="/community" className="text-amber-400 hover:underline">← Community</Link>
          {auth.isAuthenticated && (
            <Link to="/" className="text-zinc-400 hover:text-zinc-200">My projects</Link>
          )}
        </div>
      </main>
    </div>
