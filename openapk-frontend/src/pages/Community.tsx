...
// AuthProvider wraps the whole app, it just returns user=undefined.
//
// URL is the source of truth for filters so a search result can be shared
...
export function Community() {
  ...
  // Reload whenever any filter dimension changes.
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    api<CommunityReportSummary[]>(`/api/community/apk/reports${buildFeedQuery(params)}`)
  ...
  // hash short circuit instead of running FTS on a hex string.
  ...
  function submitSearch(e?: React.FormEvent) {
    ...
    if (SHA256_RE.test(trimmed)) {
      // Exact hash lookup. If there's exactly one match, jump straight to it.
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Public APK analyses contributed by the community. Browse, search by hash or keyword,
              and filter by malware type. <Link to="/terms" className="text-purple-400 hover:underline">Terms</Link>.
            </p>
          </div>
          <Link
            to="/community/researchers"
            className="shrink-0 rounded border border-purple-700/60 bg-purple-950/30 px-3 py-1.5 text-xs font-medium text-purple-200 transition hover:bg-purple-900/40"
          >
            Find researchers →
          </Link>
        </div>
        ...
        <form onSubmit={submitSearch} className="mb-4 flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-purple-600 focus:outline-none"
          />
          <button
            type="submit"
            className="rounded border border-purple-700 bg-purple-950/60 px-4 py-2 text-sm text-purple-200 hover:bg-purple-900/60"
          >
            Search
          </button>
        </form>
        ...
        {!loading && !error && items.length === 0 && (
          <div className="rounded border border-zinc-800 bg-zinc-900/40 px-6 py-12 text-center">
            ...
            {hasFilters && (
              <button onClick={clearAll} className="mt-3 text-xs text-purple-400 hover:underline">
                Clear filters
              </button>
            )}
          </div>
  ...
  function FeedCard({
    ...
    return (
      <article
        className="cursor-pointer rounded border border-zinc-800 bg-zinc-900/40 p-4 transition hover:border-purple-800 hover:bg-zinc-900/60"
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              {report.malwareType && (
                <span className="rounded-full border border-amber-700 bg-amber-950/40 px-2 py-0.5 text-[10px] uppercase tracking-wide text-amber-300">
                  {report.malwareType}
                </span>
              )}
              {report.tags.map((t) => (
                <button
                  ...
                  className="rounded-full border border-zinc-700 bg-zinc-800/60 px-2 py-0.5 text-[10px] text-zinc-300 hover:border-purple-700 hover:text-purple-200"
                >
                  #{t}
                </button>
              ))}
...
/**
 * Lightweight relative-time helper - "2h ago", "3d ago", etc. Resolves to
 * the absolute date string for anything older than 30 days so old reports
 * keep a precise timestamp.
 */
